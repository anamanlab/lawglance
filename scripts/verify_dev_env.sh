...
if [[ $failures -eq 0 ]]; then
  if UV_PYTHON="$PYTHON_BIN" uv run python - <<'PY'
import importlib
...
PY
  ...
  if UV_PYTHON="$PYTHON_BIN" uv run ruff --version >/dev/null 2>&1; then
    ok "ruff is available"
  else
    fail "ruff is unavailable in environment"
  fi

  if UV_PYTHON="$PYTHON_BIN" uv run pytest --version >/dev/null 2>&1; then
